...
### コーチング改訂リマインダー(初期構想の「月1程度のプラン改訂」を運用に乗せる)
- アプリは「処方箋のスケジューラ+記録係」に徹する(§2.1)。プラン設計・体調反映・メニュー変更はアプリではなく月次のClaudeセッションが担う。アプリ側はその「改訂の時期」を知らせるだけ
- `getCoachingReminder(today)`([src/features/gym/lib/program.ts](src/features/gym/lib/program.ts)): プログラムの `validUntil` を基準に判定(残り5日以内=due-soon / 期限切れで次未設定=overdue / 次世代あり=促さない)。周期はコーチングが決めた有効期間に自動追従(固定周期を持たない)
- UIは `src/features/gym/CoachingReminder.tsx`(ジムタブ最上部・×で閉じる)。その場で G-F11 サマリーを出力できる。出力ロジックは `lib/coachingExport.ts` に共通化(KPIタブと共用)
- ics にも有効期限日の「コーチングセッション」イベントを1件追加(`buildProgramIcs`、G-F7の延長)

### バックアップ(§5)
- `src/core/backup.ts`(非UI): `exportBackup`/`downloadBackup`/`importBackup`。`db.tables` を走査して全テーブルを動的に入出力(スキーマ増加に自動追従)。復元は clear→bulkPut で置き換え(自動採番テーブルも id ごと復元)。`schemaVersion`(db.verno)で新版バックアップを拒否
- UIは `src/features/backup/BackupPanel.tsx`(バイタルタブの最下部)。健康データ取込(ImportPanel/registry)とは別の入口で、相互に誤投入は弾かれる

未着手事項: 通知/Web Push(§4.4)、睡眠ステージ表示。
