...
4. フェーズ3: ④習慣化(docs/habit-algorithm.md のフェーズ表に従う)
   - 第1セッション ← **完了**: `packages/habit-engine/` のエンジン(純粋関数+テスト)。UIはまだ無い
   - 次セッション: 習慣タブのUI(H-F1〜H-F7)+ IndexedDB永続化(app層)。UIは `packages/habit-engine` の公開API(index.ts)経由で使う
...
未着手事項: 習慣タブUI(H-F1〜H-F7)、習慣のIndexedDB永続化、通知/Web Push(§4.4)、バックアップJSON入出力(§5)、睡眠ステージ表示。
